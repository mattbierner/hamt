...
var hash, empty, tryGetHash, tryGet, getHash, get, hasHash, has, setHash, set, modifyHash, modify, removeHash, remove,
        fold, count, pairs, keys, values, constant = (function(x) {
            return (function() {
                return x;
            });
        }),
...
var Leaf__lookup, Collision__lookup, IndexedNode__lookup, ArrayNode__lookup, Leaf__modify, Collision__modify,
        IndexedNode__modify, ArrayNode__modify, Leaf__fold, Collision__fold, IndexedNode__fold, ArrayNode__fold, Leaf =
        (function(hash0, key, value) {
        return ({
            hash: hash0,
            key: key,
            value: value,
    ...
    Collision = (function(hash0, children) {
        return ({
            hash: hash0,
            children: children,
            lookup: Collision__lookup,
            fold: Collision__fold,
            modify: Collision__modify
        });
    }),
    IndexedNode = (function(mask0, children) {
        return ({
            mask: mask0,
            children: children,
            lookup: IndexedNode__lookup,
            fold: IndexedNode__fold,
    ...
    ArrayNode = (function(count, children) {
        return ({
            count: count,
            children: children,
            lookup: ArrayNode__lookup,
            fold: ArrayNode__fold,
    ...
    isLeaf = (function(node) {
        return (((node === empty) || node.key) || node.hash);
    }),
    ...
    mergeLeaves = (function(shift, n1, n2) {
        var h1 = n1.hash,
            h2 = n2.hash,
            subH1, subH2;
        return ((h1 === h2) ? Collision(h1, [n2, n1]) : ((subH1 = ((h1 >>> shift) & mask)), (subH2 = ((h2 >>> shift) &
            mask)), IndexedNode(((1 << subH1) | (1 << subH2)), ((subH1 === subH2) ? [mergeLeaves((shift + 5),
    ...
    updateCollisionList = (function(h, list, f, k) {
        ...
        return ((nothing === v) ? arraySpliceOut(i, list) : arrayUpdate(i, Leaf(h, k, v), list));
    });
(Leaf__lookup = (function(_, _0, k) {
    var self = this;
    return ((k === self.key) ? self.value : nothing);
}));
(Collision__lookup = (function(_, h, k) {
    var self = this;
    if ((h === self.hash)) {
...
(IndexedNode__lookup = (function(shift, h, k) {
    var self = this,
        frag = ((h >>> shift) & mask),
        bit = (1 << frag),
        bitmap;
...
(ArrayNode__lookup = (function(shift, h, k) {
    var self = this,
        frag = ((h >>> shift) & mask),
...
(Leaf__modify = (function(shift, f, h, k) {
    var v, v0, self = this;
    return ((k === self.key) ? ((v = f(self.value)), ((nothing === v) ? empty : Leaf(h, k, v))) : ((v0 = f()), (
        (nothing === v0) ? self : mergeLeaves(shift, self, Leaf(h, k, v0)))));
}));
(Collision__modify = (function(shift, f, h, k) {
...
(IndexedNode__modify = (function(shift, f, h, k) {
    var __o = this,
        mask0 = __o["mask"],
    ...
    return ((!bitmap0) ? empty : (removed ? (((children.length <= 2) && isLeaf(children[(indx ^ 1)])) ?
        children[(indx ^ 1)] : IndexedNode(bitmap0, arraySpliceOut(indx, children))) : (added ? ((
        children.length >= MAX_INDEX_NODE) ? expand(frag, child, mask0, children) : IndexedNode(
        bitmap0, arraySpliceIn(indx, child, children))) : IndexedNode(bitmap0, arrayUpdate(indx,
        child, children)))));
}));
(ArrayNode__modify = (function(shift, f, h, k) {
    var __o = this,
        count = __o["count"],
        children = __o["children"],
        frag = ((h >>> shift) & mask),
    ...
    return (((((!child) || (child === empty)) || (child && child.__hamt_isEmpty)) && (!(((!newChild) || (
        newChild === empty)) || (newChild && newChild.__hamt_isEmpty)))) ? ArrayNode((count + 1),
        arrayUpdate(frag, newChild, children)) : (((!(((!child) || (child === empty)) || (child && child.__hamt_isEmpty))) &&
        (((!newChild) || (newChild === empty)) || (newChild && newChild.__hamt_isEmpty))) ? (((count -
        1) <= MIN_ARRAY_NODE) ? pack(frag, children) : ArrayNode((count - 1), arrayUpdate(frag,
        empty, children))) : ArrayNode(count, arrayUpdate(frag, newChild, children))));
}));
(empty.modify = (function(_, f, h, k) {
    var v = f();
    return ((nothing === v) ? empty : Leaf(h, k, v));
}));
...
(Leaf__fold = (function(f, z) {
    var self = this;
    return f(z, self);
}));
(Collision__fold = (function(f, z) {
    var __o = this,
        children = __o["children"];
    return children.reduce(f, z);
}));
(IndexedNode__fold = (function(f, z) {
    var __o = this,
        children = __o["children"],
        z1 = z;

...
const updateCollisionList = (h, list, f, k) => {
    ...
    for (let i = 0; i < len; ++i) {
        ...
        if (child.key === k) {
            ...
            return newValue === nothing
                ...
                :arrayUpdate(i, Leaf(h, k, newValue), list);   
        }
    }
    ...
    return newValue === nothing
        ?list
        :arrayUpdate(len, Leaf(h, k, newValue), list);
};
...
const tryGetHash = hamt.tryGetHash = (alt, hash, key, map) => {
    const v = _lookup(map._root, hash, key);
...
/* Fold
 ******************************************************************************/
const Leaf__fold = function(f, z) {
    return f(z, this.value, this.key);
};

empty._fold = (f, z) => z;

const IndexedNode__fold = function(f, z) {
    const children = this.children;
    for (let i = 0, len = children.length; i < len; ++i) {
        const c = children[i];
        z = c.type === LEAF
...
const ArrayNode__fold = function(f, z) {
    const children = this.children;
    for (let i = 0, len = children.length; i < len; ++i) {
        const c = children[i];
        if (c && !isEmptyNode(c))

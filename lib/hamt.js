...
const app = hamt.app = (k) => {
    return k();
};

var lazyVisitChildren = function(len, children, i, f, k) {
    for (; i < len; ++i) {
        var child = children[i];
        if (child && !isEmptyNode(child))
            return Cont(child, f,
                () => lazyVisitChildren(len, children, i + 1, f, k));
    }
    return app(k);
}; 
...
hamt.traverse = (f, map) =>
    trampoline(lazyVisit(map.root, (value, key, k) => {
        var i = true;
        var z = () => i ? app(k) : trampoline(app(k));
        var x = f(value, key, z);
        i = false;
        return x;
    }, () => 0))

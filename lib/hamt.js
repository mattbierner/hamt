...
*/
const updateCollisionList = (h, list, f, k, defaultValue) => {
    let target;
    let i = 0;
    ...
    const v = target ? f(target.value) : f(defaultValue);
    return v === nothing
...
Collision.prototype._modify = function(shift, f, h, k, defaultValue) {
    if (h === this.hash) {
        const list = updateCollisionList(this.hash, this.children, f, k, defaultValue);
...
IndexedNode.prototype._modify = function(shift, f, h, k, defaultValue) {
    ...
    const child = current._modify(shift + SIZE, f, h, k, defaultValue);
...
ArrayNode.prototype._modify = function(shift, f, h, k, defaultValue) {
    ...
    const newChild = (child || empty)._modify(shift + SIZE, f, h, k, defaultValue);
...
empty._modify = (_, f, h, k, defaultValue) => {
    const v = f(defaultValue);
    return v === nothing ? empty : new Leaf(h, k, v);
};
...
*/
const modifyHash = hamt.modifyHash = (f, hash, key, map, defaultValue) =>
    new Map(map.root._modify(0, f, hash, key, defaultValue));
...
/**
    ...
    @see `modifyHash`
*/
const modify = hamt.modify = (f, key, map, defaultValue) =>
    modifyHash(f, hash(key), key, map, defaultValue);

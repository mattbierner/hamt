...
/**
    ...
    Returns a map with the modified value. Does not alter `map`.
*/
const setHash = hamt.setHash = (value, hash, key, map) =>
 ...
 ******************************************************************************/
const appk = k =>
    k && lazyVisitChildren(k.len, k.children, k.i, k.k);
 
/**
    Recursively visit all values stored in an array of nodes lazily.
*/
var lazyVisitChildren = (len, children, i, k) => {
    while (i < len) {
        var child = children[i++];
        if (child && !isEmptyNode(child))
            return lazyVisit(child, {len: len, children: children, i: i, k: k});
    }
    return appk(k);
};
...
*/
hamt.forEach = (f, m) =>
    fold((_, value, key) => f(value, key), m);
